...
const RAN_GEN_TOP = r => {
  ...
  if ('Uint8Array' in this && 'crypto' in this && r <= 65536) (t = new Uint8Array(r)), this.crypto.getRandomValues(t);
  else {
    t = new Array(r);
...
const RAN_GEN_1 = r => {
  let t = '';
  for (let _ = 0; _ < r; _++) t += possible.charAt(Math.floor(Math.random() * possible.length));
  return t;
};

const RAN_GEN_2 = (r, t = '') => {
  for (; r--; ) t += String.fromCharCode(((t = (62 * Math.random()) | 0), (t += t > 9 ? (t < 36 ? 55 : 61) : 48)));
  return t;
};
...
const RAN_GEN_4 = [...Array(24)].map(() => possible[~~(Math.random() * possible.length)]).join('');

const RAN_GEN_4_C = [...Array(16)].map(() => possible[~~(Math.random() * possible.length)]).join('');
